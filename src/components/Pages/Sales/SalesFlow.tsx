...
function SalesFlow() {
      ...
      <div className="overflow-y-auto max-h-[70vh]">
        <ul className="space-y-4">
          {data.map((item, index) => {
            ...
            return (
              <li key={index} className="border-l border-purple-800 pl-4 ml-2">
                <div className="text-black dark:text-white">
                  <p className="font-semibold text-purple-800 dark:text-purple-400">{item.key}</p>
                  <p className="text-sm">
                    💰 {dollar} <span className="text-green-600  dark:text-green-400">{percent}</span>
                  </p>
                </div>
  ...
  return (
    <div className="bg-white rounded-xl dark:bg-gray-900 min-h-screen w-full text-gray-900 dark:text-white">
