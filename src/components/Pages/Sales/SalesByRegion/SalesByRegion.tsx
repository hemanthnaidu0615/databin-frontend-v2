...
const formatValue = (val: number) => val >= 1_000_000 ? `${(val / 1_000_000).toFixed(1)}M` : val >= 1_000 ? `${(val / 1_000).toFixed(1)}K` : val.toString();
...
export const SalesByRegion: React.FC = () => {
  ...
  useEffect(() => {
    const fetchTopStates = async () => {
      ...
      try {
        const topStatesResponse = await axiosInstance.get<{ state_name: string; state_revenue: number }[]>(
  ...
  const fetchData = async (params: any) => {
    ...
    try {
      const response = await axiosInstance.get(`/sales-by-region?${queryParams.toString()}`);
      const responseData = response.data as { data: StateData[]; count: number };
      return {
  ...
  const renderMobileCard = (row: StateData, index: number) => (
    <div
      key={index}
      className={`p-4 rounded-md border cursor-pointer ${theme === "dark" ? "bg-gray-800 border-gray-700" : "bg-white border-gray-200"
        }`}
      onClick={() => setExpandedState(expandedState === row.state_name ? null : row.state_name)}
    >
      <div className="flex justify-between items-center">
        ...
        <div className="flex items-center gap-2">
          ...
          {expandedState === row.state_name ? <FaChevronUp /> : <FaChevronDown />}
        </div>
      </div>
      ...
      <div className="flex flex-col lg:flex-row gap-6 py-4 items-center lg:items-start">
            ...
            <div className="flex flex-col gap-1">
              {markersList.map((item) => (
                <span key={item.color} className="flex items-center gap-2">
                  <div className="rounded-full h-[9px] w-[9px]" style={{ backgroundColor: item.color }}></div>
        ...
        <div className="w-full lg:w-1/2 mt-6 lg:mt-0">
          ...
          <BaseDataTable<StateData>
            fetchData={fetchData}
            columns={columns}
            mobileCardRender={renderMobileCard} field={"state_name"} header={""} />
        </div>
      </div>
    </div>
  );
