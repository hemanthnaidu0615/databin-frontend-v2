...
// const RecentShipmentsTable: React.FC<Props> = ({ selectedCarrier, selectedMethod }) => {
//   const [selectedShipment, setSelectedShipment] = useState<any | null>(null);
//   const [visible, setVisible] = useState(false);

//   const filteredData = mockData.filter((item) => {
//     const matchCarrier = selectedCarrier ? item.carrier === selectedCarrier : true;
//     const matchMethod = selectedMethod ? item.method === selectedMethod : true;
//     return matchCarrier && matchMethod;
//   });
...
//       <div><strong>Shipping Method:</strong> {shipment.method}</div>
//       <div><strong>Status:</strong> <Tag value={shipment.status} severity={getStatusSeverity(shipment.status)} /></div>
//       <div><strong>Ship Date:</strong> {shipment.shipDate}</div>
//       <div><strong>Estimated Delivery:</strong> {shipment.delivery}</div>
//       <div><strong>Origin:</strong> {shipment.origin}</div>
//       <div><strong>Destination:</strong> {shipment.destination}</div>
//       <div><strong>Cost:</strong> {shipment.cost}</div>
//     </div>
//   );
...
const RecentShipmentsTable: React.FC<Props> = ({ selectedCarrier, selectedMethod }) => {
  ...
  const mobilePageData = filteredData.slice(mobileFirst, mobileFirst + mobileRows);
    ...
    <div className="flex flex-col flex-1 h-full overflow-hidden rounded-xl border border-gray-200 bg-white px-6 pb-6 pt-4 dark:border-gray-800 dark:bg-white/[0.03]">
      ...
      <div className="block md:hidden space-y-4">
        ...
        {filteredData.length > mobileRows && (
          <div className="flex items-center justify-between mt-4 text-sm text-gray-700 dark:text-gray-100">
            <button
              onClick={() => setMobileFirst((prev) => Math.max(0, prev - mobileRows))}
            ...
            <span>
              Page {Math.floor(mobileFirst / mobileRows) + 1} of {Math.ceil(filteredData.length / mobileRows)}
            </span>
            <button
              onClick={() =>
                setMobileFirst((prev) =>
                  prev + mobileRows < filteredData.length ? prev + mobileRows : prev
                )
              }
